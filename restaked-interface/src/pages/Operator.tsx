...
import ShareChart from "@/components/ui/Charts/ShareChart";

import FAQ from "@/components/ui/Portfolio/FAQ";
import { fetchDataForOperator } from "@/services/DashboardServices";
import { useEffect, useState } from "react";
import { useParams } from "react-router-dom";

export default function Operator() {
  const { address: operatorAddress } = useParams(); // Access the address parameter
  const [chartData, setChartData] =
    useState<{ shares: string; timeStamp: string }[]>();
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        let data: any;
        if (operatorAddress) {
          data = await fetchDataForOperator(operatorAddress);
          let chartObject = data.datasets[0].data.map(
            (dataSet: any, index: number) => {
              return {
                shares: dataSet,
                timeStamp: data.labels[index],
              };
            }
          );
      ...
      <div className="px-[20%] py-6">
        ...
        <div className="pl-[20%] pr-[20%]">
          <div className="flex justify-center">
            <p className="mt-2 mb-10 text-2xl text-gray-300">
              Share Data Over Time
            </p>
          </div>
          {chartData && <ShareChart data={chartData} />}
        </div>
